...
- Managed subagent:
  - `session-drain-chunk` — installed at `~/.pi/agent/agents/session-drain-chunk.md`, allowed to call `session_drain_transcript`, and limited to one assigned transcript chunk.

Chunk subagents report durable memory candidates only. The parent agent reviews chunk reports, edits memory, and marks sessions.
